...
const CreatePost = () => {
  ...
  const [loadingText, setLoadingText] = useState(''); // Thêm state để báo trạng thái cho user
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 🌟 CẤU HÌNH NÉN ẢNH
      const options = {
        maxSizeMB: 2,             // Ép dung lượng tối đa xuống 2MB/ảnh
        maxWidthOrHeight: 1920,   // Giữ nguyên chuẩn Full HD (1920px)
        useWebWorker: true        // Dùng Web Worker để web không bị đơ khi nén
      };

      // 🌟 NÉN TỪNG FILE TRƯỚC KHI ĐẨY VÀO FORMDATA
      for (let i = 0; i < imageFiles.length; i++) {
        const file = imageFiles[i];
        setLoadingText(`ĐANG NÉN ẢNH ${i + 1}/${imageFiles.length}...`); // Cập nhật text UI
        
        try {
          const compressedFile = await imageCompression(file, options);
          // Gắn file đã nén vào form, giữ nguyên tên gốc
          formData.append('Images', compressedFile, file.name);
          console.log(`Đã nén ${file.name}: ${file.size/1024/1024}MB -> ${compressedFile.size/1024/1024}MB`);
        } catch (compressError) {
          console.error("Lỗi nén ảnh, dùng ảnh gốc:", compressError);
          // Rủi ro nén lỗi thì xài tạm ảnh gốc
          formData.append('Images', file, file.name);
        }
      }

      setLoadingText('ĐANG ĐẨY ẢNH LÊN SERVER...');
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            ...
            <label className="flex flex-col items-center justify-center aspect-square border-2 border-dashed border-white/10 rounded-2xl cursor-pointer hover:border-[#BDE8F5]/50 hover:bg-[#BDE8F5]/5 transition-all group shadow-sm">
              ...
              <input type="file" className="hidden" accept="image/*" multiple onChange={handleImageChange} disabled={loading} />
            </label>
          </div>
          ...
          <div className="bg-[#0F172A]/50 backdrop-blur-xl border border-white/5 p-8 rounded-[2rem] space-y-6 shadow-2xl">
            ...
            <button 
              ...
              className={`w-full py-4 rounded-2xl font-black flex items-center justify-center gap-2 transition-all active:scale-95 ${
                loading 
                  ? 'bg-[#1e293b] text-gray-400 cursor-not-allowed border border-white/5' 
