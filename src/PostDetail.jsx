...
const PostDetail = () => {
  ...
  const handleLike = async () => {
    try {
      ...
      setIsLiked(!isLiked); 
      setLikeCount(prev => isLiked ? prev + 1 : prev - 1);
      if(error.response?.status === 401) alert("Bạn cần đăng nhập để thả tim nhé!");
    }
  };
  ...
  const handleComment = async (e) => {
    ...
    try {
      const response = await axiosClient.post(`/Posts/${id}/comment`, { Text: commentText });
      const newCmt = response.data !== undefined ? response.data : response;
      setComments([...comments, newCmt]);
      setCommentText('');
    } catch (error) {
      if(error.response?.status === 401) alert("Bạn cần đăng nhập để bình luận nhé!");
    }
  };
  ...
  if (!post) return <div className="min-h-screen bg-photo-black flex items-center justify-center text-white">Không tìm thấy bài đăng</div>;

  const images = post.photos?.length > 0 
    ? post.photos.map(p => p.url.replace("http://", "https://")) 
    : [FALLBACK_IMAGE];

  // Chuẩn hóa link avatar photographer
  const photographerAvatar = post.photographerAvatar?.replace("http://", "https://");
        ...
        <div className="flex-1 glass rounded-3xl overflow-hidden flex flex-col md:flex-row border border-white/10 shadow-2xl">
          ...
          <div className="w-full md:w-[40%] flex flex-col bg-gradient-to-b from-white/[0.05] to-transparent border-l border-white/10">
            <div className="p-5 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
              ...
              >
                {/* 🌟 ĐÃ FIX: Phần hiển thị Avatar thật */}
                <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-photo-gold to-yellow-600 p-[1.5px] shadow-lg overflow-hidden">
                   {photographerAvatar ? (
                     <img 
                     ...
                     <div className="w-full h-full rounded-full bg-black flex items-center justify-center font-black text-photo-gold text-xs">
              ...
              <div className="flex items-center gap-2">
                <button className="text-sm font-bold bg-white/10 hover:bg-photo-gold hover:text-black px-4 py-2 rounded-full transition-colors">Liên hệ</button>
            ...
            <div className="border-t border-white/10 p-5 bg-white/[0.02]">
              <div className="flex items-center gap-4 mb-4">
                ...
                <button className="ml-auto hover:text-photo-gold transition-colors"><Share2 size={26} /></button>
              </div>
              ...
              <form onSubmit={handleComment} className="flex items-center relative">
                ...
                />
                <button type="submit" disabled={!commentText.trim()} className="absolute right-3 text-photo-gold disabled:text-gray-600 hover:scale-110 transition-transform">
                  <Send size={20} />
                </button>
              </form>
            </div>
